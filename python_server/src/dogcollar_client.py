...
class DogCollarClient:
    ...
    def is_connected(self) -> bool:
        try:
            ...
            logger.error(f"Unexpected error while checking connection: {str(e)}")
            return None
    ...
    def get_file_list(self) -> list[str]:
        ...
        try:
            ...
            logger.error(f"HTTP error while retrieving file list: {e.response.status_code}")
            return []
        except requests.exceptions.RequestException as e:
            logger.error(f"Network error while retrieving file list: {str(e)}")
            return []
        except Exception as e:
            logger.error(f"Unexpected error while retrieving file list: {str(e)}")
            return []
        ...
        try:
            ...
            logger.error(f"Failed to parse file list: {str(e)}")
            return []
    ...
    def download_file(self, file_name: str) -> bool:
        ...
        if not file_name or not isinstance(file_name, str):
            logger.error("Invalid or empty file name provided.")
            return False
        ...
        try:
            ...
            logger.error(f"HTTP error while downloading '{file_name}': {e.response.status_code}")
            return False
        except requests.exceptions.RequestException as e:
